...
tl.1<-align(tl.1, i = NULL, j =(2:10) , align = "center", part = "all")%>%
  ...
  width(j=1, width=1.2)
tl.1<-flextable::set_caption(tl.1,caption=("Water chemistry analytes sampled as part of the Stream Assessment Survey. Table lists sampled analytes and analytical specifications."),style="Table Caption",autonum = run_autonum(seq_id = "tab",prop=officer::fp_text_lite(bold = FALSE)))
...
```{r chemgraph1, echo=FALSE, fig.width=7.5, fig.height=6, message=FALSE, warning=FALSE, fig.cap=paste(unique(chem.trans$CHEM_PARAMETER_NAME), ", " , label, sep="")}
  

if(nrow(chem.trans)>1 & grouping_for_chem=="site"){
     label="Stars at the bottom of the graph indicate an excursion of a WQS (if applicable). Axis are presented in log scale for comparison by site." 
...
if(nrow(chem.trans)>1 & grouping_for_chem=="PWL"){
  label="Boxes represent the interquartile range (25th to 75th percentiles) of the data for each site, whiskers represent the IQR +/- 1.5 times the IQR and dots indicate potential outlier values, or those outside of the IQR +/- 1.5 times the IQR. Stars at the bottom of the graph indicate an excursion of a WQS (if applicable). Axis are presented in log scale for comparison by site." 
...
```{r turb, echo=FALSE, fig.width=7.5, fig.height=6, message=FALSE, warning=FALSE, fig.cap=paste(unique(chem.no.trans$CHEM_PARAMETER_NAME), ", " , label, sep="")}
...
```{r insituChem, echo=FALSE, fig.width=7.5, fig.height=6, message=FALSE, warning=FALSE, fig.cap=paste(unique(in.situ.short$CHEM_PARAMETER_NAME), ", " , label, sep="")}
