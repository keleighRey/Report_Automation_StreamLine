...
chem.graph.site <- function(df, na.rm = TRUE, ...){
  ...
  for (i in seq_along(chem_list)) { 
    ...
    plot <- 
      ...
      if(max(temp.n.per.site$number)>1){plot<-plot+geom_boxplot()}
      if(max(temp.n.per.site$number) == 1){plot<-plot+geom_point()}
      #geom_jitter(color=high_flow_flag)
      ...
      scale_y_log10(paste(str_to_title(df.1$CHEM_PARAMETER_NAME[df.1$CHEM_PARAMETER_NAME==chem_list[i]]), df.1$CHEM_PARAMETER_UNIT[df.1$CHEM_PARAMETER_NAME==chem_list[i]], "log(10)")) + 
      xlab("PWL ID")+
...
chem.graph.continuous.site <- function(df, na.rm = TRUE, ...){
  ...
  for (i in seq_along(chem_list)) { 
    ...
    plot <- 
      ...
      if(nrow(temp.chem)>1){plot<-plot+geom_boxplot()} 
     if(nrow(temp.chem)==1){plot<-plot+geom_point()}
...
chem.graph.continuous.site <- function(df, na.rm = TRUE, ...){
  ...
  for (i in seq_along(chem_list)) { 
    ...
    plot <- 
      ...
      if(nrow(df.1)>1){plot<-plot+geom_boxplot()}
      if(nrow(df.1)==1){plot<-plot+geom_point()}
...
chem.graph.insitu.pwl <- function(df, na.rm = TRUE, ...){
  ...
  for (i in seq_along(chem_list.2)) { 
    ...
    df<-df %>% 
      arrange(order)
    dfSITE_PWL_ID<-forcats::fct_reorder(df$SITE_PWL_ID,df$order)
